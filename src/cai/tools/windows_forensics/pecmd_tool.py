...
def _format_plan_output(result: dict) -> str:
    return format_plan_output_generic(result=result, tool_name="pecmd")


def build_pecmd_plan_dict(prefetch_dir: str, execute: bool = False, timeout_s: int = 300) -> dict:
    tools_dir = _get_windows_tools_dir()
    exe_path = tools_dir / "pecmd" / "PECmd.exe"
    tool = _find_binary([exe_path])
    if not tool:
        return {
            "ok": False,
            "tool": "pecmd",
            "error": "PECmd.exe not found",
            "searched_paths": [str(exe_path)],
        }

    args = [str(tool), "-d", str(Path(prefetch_dir).resolve())]
    result = {
        "ok": True,
        "tool": "pecmd",
        "mode": "plan_only",
        "execute": execute,
        "args": args,
        "notes": "CSV output is not enabled yet.",
    }
    if execute:
...
@function_tool
def build_pecmd_plan(prefetch_dir: str, execute: bool = False, timeout_s: int = 300) -> str:
    return _format_plan_output(
        build_pecmd_plan_dict(prefetch_dir=prefetch_dir, execute=execute, timeout_s=timeout_s)
    )
