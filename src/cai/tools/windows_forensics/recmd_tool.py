...
def build_recmd_plan_dict(hive_dir: str, execute: bool = False, timeout_s: int = 300) -> dict:
    ...
    exe_path = tools_dir / "recmd" / "RECmd.exe"
    tool = _find_binary([exe_path])
    if not tool:
        return {
            "ok": False,
            "tool": "recmd",
            "error": "RECmd.exe not found",
            "searched_paths": [str(exe_path)],
        }

    args = [str(tool), "-d", str(Path(hive_dir).resolve())]
    result = {
        "ok": True,
        "tool": "recmd",
        "mode": "plan_only",
        "execute": execute,
        "args": args,
        "notes": "Batch and output options are deferred.",
    }
    if execute:
...
@function_tool
def build_recmd_plan(hive_dir: str, execute: bool = False, timeout_s: int = 300) -> str:
    return _format_plan_output(
        build_recmd_plan_dict(hive_dir=hive_dir, execute=execute, timeout_s=timeout_s)
    )