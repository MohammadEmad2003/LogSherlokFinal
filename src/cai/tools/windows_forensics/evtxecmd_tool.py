...
def _format_plan_output(result: dict) -> str:
    return format_plan_output_generic(result=result, tool_name="evtxecmd")


def build_evtxecmd_plan_dict(evtx_dir: str, execute: bool = False, timeout_s: int = 300) -> dict:
    tools_dir = _get_windows_tools_dir()
    exe_path = tools_dir / "evtxecmd" / "EvtxECmd.exe"
    tool = _find_binary([exe_path])
    if not tool:
        return {
            "ok": False,
            "tool": "evtxecmd",
            "error": "EvtxECmd.exe not found",
            "searched_paths": [str(exe_path)],
        }

    args = [str(tool), "-d", str(Path(evtx_dir).resolve())]
    result = {
        "ok": True,
        "tool": "evtxecmd",
        "mode": "plan_only",
...
@function_tool
def build_evtxecmd_plan(evtx_dir: str, execute: bool = False, timeout_s: int = 300) -> str:
    return _format_plan_output(
        build_evtxecmd_plan_dict(evtx_dir=evtx_dir, execute=execute, timeout_s=timeout_s)
    )
