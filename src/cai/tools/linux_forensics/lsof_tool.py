...
def _format_plan_output(result: dict) -> str:
    return format_plan_output_generic(result=result, tool_name="lsof")


def build_lsof_plan_dict(target: str = "", execute: bool = False, timeout_s: int = 300) -> dict:
    tools_dir = _get_linux_tools_dir()
    bin_path = tools_dir / "lsof" / "lsof"
    tool = _find_binary([bin_path, "lsof"])
    if not tool:
        return {
            "ok": False,
            "tool": "lsof",
            "error": "lsof not found",
            "searched_paths": [str(bin_path), "lsof"],
        }
    ...
    result = {
        ...
        "execute": execute,
        "args": args,
        "notes": "Target is optional and can be PID, file path, or filter token.",
    }
    if execute:
...
@function_tool
def build_lsof_plan(target: str = "", execute: bool = False, timeout_s: int = 300) -> str:
    return _format_plan_output(
        build_lsof_plan_dict(target=target, execute=execute, timeout_s=timeout_s)
    )
