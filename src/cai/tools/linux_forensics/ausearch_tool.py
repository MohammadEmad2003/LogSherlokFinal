...
def _format_plan_output(result: dict) -> str:
    return format_plan_output_generic(result=result, tool_name="ausearch")


def build_ausearch_plan_dict(search_key: str = "", execute: bool = False, timeout_s: int = 300) -> dict:
    tools_dir = _get_linux_tools_dir()
    bin_path = tools_dir / "auditd" / "ausearch"
    tool = _find_binary([bin_path, "ausearch"])
    if not tool:
        return {
            "ok": False,
            "tool": "ausearch",
            "error": "ausearch not found",
            "searched_paths": [str(bin_path), "ausearch"],
        }

    args = [str(tool)]
    if search_key:
        args.extend(["-k", search_key])
...
@function_tool
def build_ausearch_plan(search_key: str = "", execute: bool = False, timeout_s: int = 300) -> str:
    return _format_plan_output(
        build_ausearch_plan_dict(search_key=search_key, execute=execute, timeout_s=timeout_s)
    )
