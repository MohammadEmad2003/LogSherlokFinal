...
def _format_plan_output(result: dict) -> str:
    return format_plan_output_generic(result=result, tool_name="journalctl")


def build_journalctl_plan_dict(since: str = "-24h", execute: bool = False, timeout_s: int = 300) -> dict:
    tools_dir = _get_linux_tools_dir()
    bin_path = tools_dir / "journalctl" / "journalctl"
    tool = _find_binary([bin_path, "journalctl"])
    if not tool:
        return {
            "ok": False,
            "tool": "journalctl",
            "error": "journalctl not found",
            "searched_paths": [str(bin_path), "journalctl"],
        }

    args = [str(tool), "--no-pager", "--since", since]
    result = {
        "ok": True,
        "tool": "journalctl",
        "mode": "plan_only",
        "execute": execute,
        "args": args,
        "notes": "Basic journal query by time window.",
    }
    if execute:
...
@function_tool
def build_journalctl_plan(since: str = "-24h", execute: bool = False, timeout_s: int = 300) -> str:
    return _format_plan_output(
        build_journalctl_plan_dict(since=since, execute=execute, timeout_s=timeout_s)
    )
